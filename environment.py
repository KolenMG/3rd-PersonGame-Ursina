...
class InfiniteTerrain(Entity): ########### inspired by https://github.com/Vxtzq/Openworld
    def __init__(self, camera, **kwargs):
        ...
        self.chunks = {}
        self.chunk_size = 16
        self.terrain_scale = 120
        self.height_scale = 6
        self.chunk_distance = 4 #was 2 by default
                        ...
                        for j in range(1, self.chunk_size, 4):
                            if random.randint(0, 100) > 95:
                                if heightmap[i * (self.chunk_size + 1) + j] > 1 and heightmap[i * (self.chunk_size + 1) + j - 16] - 1 < 6:
                                    ...
                                    mineral = Mineral(
    position=(i, heightmap[i * (self.chunk_size - 1) + j - 16] - 1, j),
    ore_type=ore_type,
    texture=texture,
    inventory_ui=self.inventory_ui,  # This ensures the mineral is added to the inventory when mined
)
                                    mineral.parent = new_chunk  
